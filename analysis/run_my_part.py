...
def main() -> None:
    ...
    # Centralities (on giant WCC to avoid pathological closeness/paths; PR/betw still ok)
    ...
    # Extra tool: community structure + trophic levels
    ...
    # Visualisations: centrality gradients and community-colored network
    ...
    for col, cmap in [
        ...
        plot_network_by_centrality(
            ...
            title=f"Ythan food web — nodes colored/sized by {col}",
            cmap=cmap,
        )

    # Plot network colored by best-modularity communities
    plot_network_by_communities(
        G_wcc,
        pos,
        membership=comm.membership,
        out_path=OUT / f"ythan_network_communities_{comm.method}.png",
        title=f"Ythan food web — communities ({comm.method})",
    )
